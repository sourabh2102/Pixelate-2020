...
def bot_movement(go_path,box_grabbed):
    ...
    for box in go_path:
        ...
        if (box == go_path[len(go_path) - 1]):
            ...
            for cnt in contours:
                ...
                if (area_of_cnt > thresh_cnt_area):
                    ...
                    if (r1 == r and c1 == c):
                        ...
                        cv2.circle(arena, (cenx, ceny), 2, (0, 255, 0), 2)
                        cenx+=0
                        ceny+=0
                        cv2.imshow("center of white box",arena)
                        #cv2.waitKey(0)
                        cv2.destroyWindow("center of white box")
                        #flag = True
                        break
            if (color_mat[r][c] == 2):
                flag2 = False
            ...
            print("updated_min_thresh_dis:",min_thres_dis)
            #if flag2 == True:
             #   destination = np.array([cenx,ceny])
        while (dis > min_thres_dis):  # threshold distance by calibertaion
            print("dis :", dis)
    ...
    if(flag2):
        ...
        ret,img=cap.read()
        arena = img[int(roi[1]):int(roi[1] + roi[3]), int(roi[0]):int(roi[0] + roi[2])]
        cv2.circle(arena,(destination[0],destination[1]),2,(0,255,0),4)
        print(destination[0],destination[1])
        cv2.imshow('center while grabbing :',arena)
        #cv2.waitKey(0)
        #cv2.destroyWindow("center while grabbing :")
        ser.write(b'D')
        sleep(1)
    elif(flag):
        box_grabbed = False
    ...
    for j in range(n):
        ...
        if (color_mat[i][j] < 2):
            ...
            if (ry < n):
                ...
                cell_wei[cell_num[i][j]].append(1)
        if (color_mat[i][j] ==2 or color_mat[i][j]==3):
            if (upx >= 0):
                cell_adj[cell_num[i][j]].append(cell_num[upx][j])
